...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (servletPath.matches("/css.*")) {
            ...
            //sessionからログインしている従業員の情報を取得(EmployeeViewへキャスト
            EmployeeView ev = (EmployeeView) session.getAttribute(AttributeConst.LOGIN_EMP.getValue());

            /*「ログイン画面表示」と「ログイン」については、ログインしていなくても実行できる必要があるので、
             * リダイレクトの対象から除外しています。
             */
            if (ev == null) { //ログイン状態かどうかチェック
                //未ログイン
                ...
                if (!(ForwardConst.ACT_AUTH.getValue().equals(action)
                    ...
                    return;
                } /*http://localhost:8080/daily_report_system/?action=Top&command=index topにアクセスしようとしても
                ...
                */
                ...
                if (ForwardConst.ACT_AUTH.getValue().equals(action)) {
                    ...
                    if (ForwardConst.CMD_SHOW_LOGIN.getValue().equals(command)) {
                        //ログイン済みでログインページの表示をする必要はないのでトップ画面にリダイレクト
                        ((HttpServletResponse) response).sendRedirect(
                                contextPath
